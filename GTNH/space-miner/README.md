...
### `target_config.lua` — Stock Targets

This is the only file normally edited to choose what MEDINA mines. Its default
target is calculated in the same style as `space-pumping`:
...
targets.items = {
  ...
  ["Nether Star"] = "1m",             -- absolute override
  ["Diamond"] = { cellType="4096k", cellCount=2, safetyMargin=0.10 },
}
```

The formula is `cell capacity × cell count × (1 − safety margin)`. One
`16384k` item cell at a 20% margin therefore produces a target of 106,535,321
...
1. Build a needs list from the targets generated from `target_config.lua` — items below their threshold, sorted by the chosen priority mode.
...
Copy both shared files to **every** computer in the system:
...
Project updates may replace `config.lua`; keep your own `target_config.lua`.
...
Each telem node needs only its own script and the two shared config files:
...
4. Broker dispatch cycle (every 1 second):
   ...
   b. Build needs list from target_config.lua (items below threshold)
