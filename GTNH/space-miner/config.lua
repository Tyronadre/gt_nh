...
-- Convert quantity strings such as 100k, 1.5m, or 2b to item counts.
local function qty(value)
  if type(value) == "number" then return value end
  if type(value) ~= "string" then return nil end

  local normalized = string.lower((value:gsub("%s+", "")))
  local number, suffix = string.match(normalized, "^(%d+%.?%d*)([kmbt]?)$")
...
-- Add or remove entries freely; only items enabled in target_config.lua will
-- actually trigger mining jobs.
--
...
--------------------------------------------------------------------------------
-- 10. DUST STOCK TARGETS
-- User-editable targets live in /home/target_config.lua. This section validates
-- that file and converts its item map into the config.conditions list consumed
-- by the broker and dust telemetry node.
--------------------------------------------------------------------------------
local function loadTargetConfig()
  local paths = { "/home/target_config.lua", "target_config.lua" }
  local errors = {}
  ...
  error("Unable to load target_config.lua. Run install-medina again or copy " ..
        "target_config.lua to /home.\n" .. table.concat(errors, "\n"))
end

local function setting(itemSettings, targetSettings, key, fallback)
  if type(itemSettings) == "table" and itemSettings[key] ~= nil then
    return itemSettings[key]
  end
  if targetSettings[key] ~= nil then return targetSettings[key] end
  return fallback
end

local function resolveTarget(itemName, itemSettings, targetSettings)
  if itemSettings == false then return nil end
  if type(itemSettings) == "table" and itemSettings.enabled == false then return nil end

  local explicitTarget
  local hasExplicitTarget = false
  if type(itemSettings) == "number" or type(itemSettings) == "string" then
    explicitTarget = itemSettings
    hasExplicitTarget = true
  elseif type(itemSettings) == "table" then
    explicitTarget = itemSettings.target
    hasExplicitTarget = itemSettings.target ~= nil
  elseif itemSettings ~= true then
    error("Invalid target entry for '" .. itemName ..
          "': use true, false, a quantity, or a settings table")
  end

  local target
  if hasExplicitTarget then
    target = qty(explicitTarget)
    assert(target, "Invalid target quantity '" .. tostring(explicitTarget) ..
           "' for '" .. itemName .. "'")
  end

  if not target then
    local overrideSetting = targetSettings.maxTargetOverride or 0
    local globalOverride = qty(overrideSetting)
    assert(globalOverride, "Invalid maxTargetOverride quantity '" ..
           tostring(overrideSetting) .. "' in target_config.lua")
    assert(globalOverride >= 0,
           "maxTargetOverride in target_config.lua must not be negative")
    if globalOverride and globalOverride > 0 then
      target = globalOverride
    else
      local cellType = setting(itemSettings, targetSettings, "cellType",
                               targetSettings.currentCellType)
      local cellCount = setting(itemSettings, targetSettings, "cellCount", 1)
      local margin = setting(itemSettings, targetSettings, "safetyMargin", 0)
      local capacities = targetSettings.CELL_CAPACITIES or {}
      local capacity = capacities[cellType]

      assert(type(capacity) == "number" and capacity > 0,
             "Unknown or invalid cellType '" .. tostring(cellType) ..
             "' for target '" .. itemName .. "'")
      assert(type(cellCount) == "number" and cellCount >= 1 and
             cellCount == math.floor(cellCount),
             "cellCount for '" .. itemName .. "' must be a positive integer")
      assert(type(margin) == "number" and margin >= 0 and margin < 1,
             "safetyMargin for '" .. itemName .. "' must be >= 0 and < 1")

      target = capacity * cellCount * (1 - margin)
    end
  end

  assert(type(target) == "number" and target > 0,
         "Target for '" .. itemName .. "' must be greater than zero")
